...
int main(int argc, char *argv[])
{
   ...
   while(quit != 'S')
   {
      //bytes = send(new_sockfd, "CONEXAO OK HTTP 1.1 200{ \"message\": \"ok\", \"code\": 200 }", 55, 0);
      bytes = send(new_sockfd, "ok", 2, 0);
      //ocorreu um erro = -1
      if(bytes < 0)
      {
         perror("server_send:::");
      ...
      bytes = recv(new_sockfd, buffer, 500, 0);
